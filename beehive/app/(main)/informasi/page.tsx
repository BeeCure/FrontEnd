  ...
  return (
    <main className="h-100vh w-full bg-[#FFF8E1] px-4 md:mt-1 mt-0 md:px-16 flex flex-col justify-between py-6 overflow-hidden font-inder text-[#4B2E05]">
        ...
        <div className="flex w-full md:w-auto justify-between gap-2">
          {filterButtons.map((btn) => (
            <Button 
              ...
              className={cn(
                "flex-1 md:w-32 rounded-[15px] text-[11px] md:text-lg font-bold shadow-sm transition-all h-10 border-none", filter === btn 
                ? "bg-[#4B2E05] text-[#F4B740] hover:bg-[#4B2E05]" : "bg-[#F4B740]/40 text-[#4B2E05]/60 hover:bg-[#4B2E05] hover:text-[#F4B740]" )}
            >
              {btn}
            </Button>
          ))}
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-2 md:px-10 overflow-y-auto scrollbar-hide py-4">
        {isLoading ? ( <div className="w-full h-full flex items-center justify-center"> <Loader2 className="animate-spin opacity-20" size={48} /> </div>
        ) : (
          <>
              ...
              <svg width="0" height="0" className="absolute"> 
                <defs> 
                  <clipPath id="smoothHexagon" clipPathUnits="objectBoundingBox">
                    <path d="M0.5,0.015 C0.543,0.015 0.88,0.208 0.92,0.23 C0.963,0.255 0.985,0.285 0.985,0.335 L0.985,0.665 C0.985,0.715 0.963,0.745 0.92,0.77 C0.88,0.792 0.543,0.985 0.5,0.985 C0.457,
                    0.985 0.12,0.792 0.08,0.77 C0.037,0.745 0.015,0.715 0.015,0.665 L0.015,0.335 C0.015,0.285 0.037,0.255 0.08,0.23 C0.12,0.208 0.457,0.015 0.5,0.015 Z" />
                  </clipPath> 
                </defs> 
              </svg>
              {currentItems.length > 0 ? currentItems.map((bee, idx) => (
                <motion.div 
                  key={bee.id} {...anim.base(idx * 0.05, 30, 0.4)} 
                  ...
                  >
                    <Image 
                      src={bee.images?.bodyShape || "/Image/Lebah1.png"} 
                      alt={bee.name} fill unoptimized priority={idx < 4} 
                      className="object-cover transition-transform duration-700 group-hover:scale-110" 
                    />
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors duration-500" />
                  </div>
                  <div className="w-[85%] bg-[#F4B740] rounded-[22px] py-3 px-2 text-center -mt-14 md:-mt-17 relative z-10 shadow-lg border border-white/20 
                    transition-all duration-500 group-hover:-translate-y-2 group-hover:bg-[#4B2E05]">
      ...
      <motion.div {...anim.base(0, 20)} className="mt-0">
        {userRole && totalPages > 1 && (
          <Pagination>
            <PaginationContent>
              <PaginationItem><PaginationPrevious href="#" onClick={(e) => { e.preventDefault(); if(currentPage > 1) setCurrentPage(currentPage - 1) }} className={cn("text-[#4B2E05] font-bold", currentPage === 1 
                ? "opacity-30 pointer-events-none" : "hover:bg-[#F4B740]/20")} /></PaginationItem>
              {Array.from({ length: totalPages }).map((_, i) => (
                <PaginationItem key={i} className="hidden sm:block">
                  <PaginationLink href="#" isActive={currentPage === i + 1} onClick={(e) => { e.preventDefault(); setCurrentPage(i + 1); }} className={cn("rounded-[10px] font-bold transition-all border-none", 
                    currentPage === i + 1 ? "bg-[#4B2E05] text-[#F4B740] hover:bg-[#4B2E05]" : "text-[#4B2E05] hover:bg-[#F4B740]/20")}>{i + 1}</PaginationLink>
                </PaginationItem>
              ))}
              <PaginationItem><PaginationNext href="#" onClick={(e) => { e.preventDefault(); if(currentPage < totalPages) setCurrentPage(currentPage + 1) }} className={cn("text-[#4B2E05] font-bold", 
                currentPage === totalPages ? "opacity-30 pointer-events-none" : "hover:bg-[#F4B740]/20")} /></PaginationItem>
            </PaginationContent>
          </Pagination>
        )}
      </motion.div>
