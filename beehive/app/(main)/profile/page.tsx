...
export default function ProfilePage() {
  ...
  const [user, setUser] = useState<{ name: string; email: string; phone?: string; address?: string; avatarUrl: string | null; lastLogin: string | null; } | null>(null);
  ...
  const avatarSrc = user.avatarUrl ? `${user.avatarUrl}${user.avatarUrl.includes('?') ? '&' : '?'}t=${new Date().getTime()}` : "https://github.com/shadcn.png";
            ...
            <div className="w-full space-y-1">
              <EditProfile initialData={user}>
                <div className="flex items-center gap-4 py-3 border-b-2 border-[#4B2E05] cursor-pointer hover:opacity-70 transition-all">
                  <Pencil size={22} /><span className="text-lg font-medium">Edit Profil</span>
                </div>
              </EditProfile>
              <ResetPassword>
                <div className="flex items-center gap-4 py-3 border-b-2 border-[#4B2E05] cursor-pointer hover:opacity-70 transition-all">
                  <KeyRound size={22} /><span className="text-lg font-medium">Ganti Kata Sandi</span>
                </div>
              </ResetPassword>
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <div className="flex items-center gap-4 py-3 border-b-2 border-[#4B2E05] cursor-pointer hover:opacity-70 transition-all text-red-700 md:text-[#4B2E05]">
                    <LogOut size={22} /><span className="text-lg font-medium">Keluar</span>
                  </div>
                </AlertDialogTrigger>
                <AlertDialogContent className="w-[90%] sm:w-full max-w-[450px] bg-[#FFF8E1] border-none rounded-[15px]">
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription className="text-[#4B2E05]/80">Tindakan ini akan mengakhiri sesi Anda dan Anda harus login ulang untuk mengakses sistem.</AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel className="rounded-full border-[#4B2E05] text-[#4B2E05]">Batal</AlertDialogCancel>
                    <AlertDialogAction onClick={handleLogout} className="bg-[#8E4117] hover:bg-[#7a3713] text-white rounded-full">Keluar</AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>
              {user.lastLogin && (
                <div className="flex items-center gap-2 pt-4 opacity-60">
                  <Clock size={14} /><span className="text-[10px] md:text-xs">Terakhir masuk: {user.lastLogin}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="w-full md:w-2/3 bg-[#F4B740] rounded-[15px] p-6 md:p-8 shadow-2xl">
          ...
          <div className="grid grid-cols-1 gap-3">
            <div className="space-y-1">
              ...
              <Input readOnly value={user.name} className="h-11 rounded-[15px] border-none bg-[#FFF8E1] shadow-md text-[#4B2E05] text-base px-5 focus-visible:ring-0 cursor-default" />
            </div>
            <div className="space-y-1">
              <Label className="text-base font-bold text-[#4B2E05] ml-1">Email</Label>
              <Input readOnly value={user.email} className="h-11 rounded-[15px] border-none bg-[#FFF8E1] shadow-md text-[#4B2E05] text-base px-5 focus-visible:ring-0 cursor-default" />
            </div>
            <div className="space-y-1">
              <Label className="text-base font-bold text-[#4B2E05] ml-1">No Telepon</Label>
              <Input readOnly value={user.phone || "-"} className="h-11 rounded-[15px] border-none bg-[#FFF8E1] shadow-md text-[#4B2E05] text-base px-5 focus-visible:ring-0 cursor-default" />
            </div>
            <div className="space-y-1">
              <Label className="text-base font-bold text-[#4B2E05] ml-1">Alamat</Label>
              <Input readOnly value={user.address || "-"} className="h-11 rounded-[15px] border-none bg-[#FFF8E1] shadow-md text-[#4B2E05] text-base px-5 focus-visible:ring-0 cursor-default" />
            </div>
          </div>
        </div>
      </div>
