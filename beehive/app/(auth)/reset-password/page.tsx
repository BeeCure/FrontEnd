  ...
  return (
    <motion.div {...anim.base(0, 40)} className="flex flex-col md:flex-row w-[90%] md:w-[85%] h-auto md:h-[80vh] max-w-[1100px] bg-[#F4B740] rounded-[15px] shadow-2xl overflow-hidden border border-black/5">
      ...
      <motion.div {...anim.side(0.2, 50)} className="flex-1 flex flex-col items-center justify-center p-8 md:p-12 text-[#4B2E05]">
        <motion.h2 {...anim.base(0.3)} className="text-3xl font-bold mb-12 tracking-widest uppercase text-center leading-tight">
          Ganti Kata <br /> Sandi
        </motion.h2>
        ...
        <form onSubmit={handleResetPassword} className="w-full max-w-[340px] space-y-8">
          <motion.div {...anim.base(0.4)} className="space-y-2">
            ...
            {showPassError && <p className="text-[10px] text-red-600 font-bold ml-2 leading-tight">Minimal 8 karakter, gunakan huruf besar, kecil, dan angka</p>}
          </motion.div>
          ...
          <motion.div {...anim.base(0.5)} className="space-y-2">
            ...
            <div className="relative">
              ...
              />
              <button type="button" onClick={() => setShowConfirmPass(!showConfirmPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-2xl text-[#4B2E05]/60 hover:text-[#4B2E05]">
                {showConfirmPass ? <CiUnread /> : <CiRead />}
              </button>
            </div>
            {showError && <p className="text-xs text-red-600 font-bold ml-2">Kata sandi tidak cocok</p>}
          </motion.div>

          <motion.div {...anim.base(0.6)} className="flex justify-center pt-6">
